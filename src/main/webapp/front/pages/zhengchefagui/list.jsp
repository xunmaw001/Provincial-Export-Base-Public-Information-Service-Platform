                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="zhengchefaguiName" id="zhengchefaguiName" placeholder="政策法规名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="zhengchefaguiTypes" id="zhengchefaguiTypes" lay-filter="zhengchefaguiTypes">
                                  ...
                                  <option v-for="item in zhengchefaguiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../zhengchefagui/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.zhengchefaguiPhoto" />
                                        <h5>{{item.zhengchefaguiName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.zhengchefaguiContent|zhengchefaguiContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zhengchefaguiContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zhengchefaguiContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#zhengchefaguiName').val() != null && jquery('#zhengchefaguiName').val() != "" && jquery('#zhengchefaguiName').val() != "null" ) {
                param['zhengchefaguiName'] = jquery('#zhengchefaguiName').val();
            }
     
            var zhengchefaguiTypes = $("#zhengchefaguiTypes").find("option:selected").val();
            if (zhengchefaguiTypes != null && zhengchefaguiTypes != "" && zhengchefaguiTypes != "null") {
                param['zhengchefaguiTypes'] = zhengchefaguiTypes;
            }
                            // 获取列表数据
            http.request('zhengchefagui/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function zhengchefaguiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhengchefagui_types", 'get', {}, function (res) {
               if(res.code == 0){
