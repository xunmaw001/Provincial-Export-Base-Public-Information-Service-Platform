            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">基地企业收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="jidiqiyeName" id="jidiqiyeName" placeholder="企业名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../jidiqiye/detail.jsp?id='+item.jidiqiyeId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.jidiqiyePhoto?item.jidiqiyePhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../jidiqiye/detail.jsp?id='+item.jidiqiyeId)"><span>{{item.jidiqiyeName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteJidiqiyeCollection(item.id)">删除 {{item.jidiqiyeName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 jidiqiyeCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            jidiqiyeCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteJidiqiyeCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jidiqiyeCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#jidiqiyeName').val() != null && jquery('#jidiqiyeName').val() != "" && jquery('#jidiqiyeName').val() != "null" ) {
                param['jidiqiyeName'] = jquery('#jidiqiyeName').val();
            }
            ...
            // 当前表
                                // 获取列表数据
            http.request('jidiqiyeCollection/list', 'get', param, function(res) {
