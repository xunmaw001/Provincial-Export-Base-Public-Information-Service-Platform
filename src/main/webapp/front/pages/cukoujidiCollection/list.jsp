            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">出口基地收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="cukoujidiName" id="cukoujidiName" placeholder="名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="jidiqiyeTypes" id="jidiqiyeTypes" lay-filter="jidiqiyeTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in jidiqiyeTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../cukoujidi/detail.jsp?id='+item.cukoujidiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.cukoujidiPhoto?item.cukoujidiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../cukoujidi/detail.jsp?id='+item.cukoujidiId)"><span>{{item.cukoujidiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteCukoujidiCollection(item.id)">删除 {{item.cukoujidiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 cukoujidiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            cukoujidiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteCukoujidiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('cukoujidiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#cukoujidiName').val() != null && jquery('#cukoujidiName').val() != "" && jquery('#cukoujidiName').val() != "null" ) {
                param['cukoujidiName'] = jquery('#cukoujidiName').val();
            }
                                 
            var jidiqiyeTypes = $("#jidiqiyeTypes").find("option:selected").val();
            if (jidiqiyeTypes != null && jidiqiyeTypes != "" && jidiqiyeTypes != "null") {
                            param['jidiqiyeTypes'] = jidiqiyeTypes;
                        }
                                                
            // 当前表
                                // 获取列表数据
            http.request('cukoujidiCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function jidiqiyeTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jidiqiye_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.jidiqiyeTypesList = res.data.list;
               }
           });
       }
