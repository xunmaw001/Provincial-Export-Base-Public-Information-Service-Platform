            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">基地产品收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="jidichanpinName" id="jidichanpinName" placeholder="名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="jidichanpinTypes" id="jidichanpinTypes" lay-filter="jidichanpinTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in jidichanpinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../jidichanpin/detail.jsp?id='+item.jidichanpinId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.jidichanpinPhoto?item.jidichanpinPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../jidichanpin/detail.jsp?id='+item.jidichanpinId)"><span>{{item.jidichanpinName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteJidichanpinCollection(item.id)">删除 {{item.jidichanpinName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 jidichanpinCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            jidichanpinCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteJidichanpinCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('jidichanpinCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#jidichanpinName').val() != null && jquery('#jidichanpinName').val() != "" && jquery('#jidichanpinName').val() != "null" ) {
                param['jidichanpinName'] = jquery('#jidichanpinName').val();
            }
                 
            var jidichanpinTypes = $("#jidichanpinTypes").find("option:selected").val();
            if (jidichanpinTypes != null && jidichanpinTypes != "" && jidichanpinTypes != "null") {
                            param['jidichanpinTypes'] = jidichanpinTypes;
                        }
                                                
            // 当前表
                                // 获取列表数据
            http.request('jidichanpinCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function jidichanpinTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jidichanpin_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.jidichanpinTypesList = res.data.list;
               }
           });
       }
