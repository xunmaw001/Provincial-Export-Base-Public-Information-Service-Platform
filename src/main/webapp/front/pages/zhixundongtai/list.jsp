                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="zhixundongtaiName" id="zhixundongtaiName" placeholder="资讯动态名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="zhixundongtaiTypes" id="zhixundongtaiTypes" lay-filter="zhixundongtaiTypes">
                                  ...
                                  <option v-for="item in zhixundongtaiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../zhixundongtai/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.zhixundongtaiPhoto" />
                                        <h5>{{item.zhixundongtaiName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.zhixundongtaiContent|zhixundongtaiContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 zhixundongtaiContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zhixundongtaiContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#zhixundongtaiName').val() != null && jquery('#zhixundongtaiName').val() != "" && jquery('#zhixundongtaiName').val() != "null" ) {
                param['zhixundongtaiName'] = jquery('#zhixundongtaiName').val();
            }
     
            var zhixundongtaiTypes = $("#zhixundongtaiTypes").find("option:selected").val();
            if (zhixundongtaiTypes != null && zhixundongtaiTypes != "" && zhixundongtaiTypes != "null") {
                param['zhixundongtaiTypes'] = zhixundongtaiTypes;
            }
                            // 获取列表数据
            http.request('zhixundongtai/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function zhixundongtaiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhixundongtai_types", 'get', {}, function (res) {
               if(res.code == 0){
