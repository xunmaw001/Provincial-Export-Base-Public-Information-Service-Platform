package com.entity.view;

import com.entity.CukoujidiEntity;

import com.baomidou.mybatisplus.annotations.TableName;
import org.apache.commons.beanutils.BeanUtils;
import java.lang.reflect.InvocationTargetException;
import org.springframework.format.annotation.DateTimeFormat;
import com.fasterxml.jackson.annotation.JsonFormat;

import java.io.Serializable;
import java.util.Date;

/**
 * 出口基地
 * 后端返回视图实体辅助类
 * （通常后端关联的表或者自定义的字段需要返回使用）
 * @author 
 * @email
 * @date 2021-04-19
 */
@TableName("cukoujidi")
public class CukoujidiView extends CukoujidiEntity implements Serializable {
    private static final long serialVersionUID = 1L;
		/**
		* 类型的值
		*/
		private String jidiqiyeValue;



	public CukoujidiView() {

	}

	public CukoujidiView(CukoujidiEntity cukoujidiEntity) {
		try {
			BeanUtils.copyProperties(this, cukoujidiEntity);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}



			/**
			* 获取： 类型的值
			*/
			public String getJidiqiyeValue() {
				return jidiqiyeValue;
			}
			/**
			* 设置： 类型的值
			*/
			public void setJidiqiyeValue(String jidiqiyeValue) {
				this.jidiqiyeValue = jidiqiyeValue;
			}
















}
